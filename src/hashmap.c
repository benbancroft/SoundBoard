 ...
 */
int hashmap_get(hashmap *map, int key, void **value) {
    // Compute the hash value of the key
    unsigned int index = key % map->table_size;
 ...
 */
static int hashmap_insert_table(hashmap_entry *table, int table_size, int key,
...
static void hashmap_double_size(hashmap *map) {
    ...
    for (int i=0; i < map->table_size; i++) {
        ...
        while (entry && entry->key) {
            ...
            hashmap_insert_table(new_table, new_size, old->key,
                                 old->value, 0, 0);
...
int hashmap_put(hashmap *map, int key, void *value) {
    ...
    int new = hashmap_insert_table(map->table, map->table_size, key, value, 1, 1);
    if (new) map->count += 1;
